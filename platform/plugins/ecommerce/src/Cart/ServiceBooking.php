...
class ServiceBooking
{
    ...
    public function instance($instance = null)
    {
        ...
        $this->instance = sprintf('%s.%s', 'service_booking', $instance);
        return $this;
    }
    ...
    public function add($id, $name = null, $qty = null, $price = null, array $options = [])
    {
        ...
        $this->events->dispatch('service_booking.added', $cartItem);
    ...
    public function update($rowId, $qty)
    {
        ...
        $this->events->dispatch('service_booking.updated', $cartItem);
    ...
    public function remove($rowId)
    {
        ...
        $serviceModel = \Botble\Ecommerce\Models\ServiceBooking::find($cartItem->id);
        ...
        $this->events->dispatch('service_booking.removed', $cartItem);
    ...
    {
        return config('plugins.ecommerce.cart.database.table', 'service_booking');
    }
    ...
    {
        return str_replace('service_booking.', '', $this->instance);
    }
    ...
    public function restore($identifier)
    {
        ...
        $this->events->dispatch('service_booking.restored');
    ...
    public function services()
    {
        ...
        $cartContent = $this->instance('service_booking')->content();
        $serviceIds = $cartContent->pluck('id')->toArray();
        $services = collect([]);
        ...
        if ($serviceIds) {
            ...
            $services = app(BookedServiceInterface::class)->getBookedServices([
                'condition' => [
                    [
                        'ec_service_booking.id', 'IN', $serviceIds],
                    'ec_service_booking.status' => BaseStatusEnum::PENDING,
                ],
                'with' => $with,
            ]);
        }
        if ($services->count()) {
            foreach ($cartContent as $cartItem) {
                $service = $services->firstWhere('id', $cartItem->id);
                if (!$service) {
                    $this->remove($cartItem->rowId);
                } else {
        ...
        if ($this->services->count() == 0) {
            $this->instance('service_booking')->destroy();
        }
