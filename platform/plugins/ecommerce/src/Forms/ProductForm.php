    ...
    public function buildForm()
    {
        ...
        if($uriArray[4] == 'edit'){
            ...
            if(isset($_GET['ref_lang'])){
               $value = ProductTranslation::where('lang_code',$_GET['ref_lang'])->where('ec_products_id',$id)->first();
                $lang_code = $_GET['ref_lang'];
            }else{
            ...
            foreach ($languages as $language) {
                if($language['locale'] == $lang_code){
                    unset($language['name']);
                }
                $lang[] = isset($language['name']) ? $language['name'] : '';
                $langid[] = isset($language['locale']) ? $language['locale'] : '';
            }
        }
        $selectedCategories = [];
        if ($this->getModel()) {
        ...
        ->addCustomField('tags', TagField::class)
            ...
            ->add('min_quantity', 'number', [
                ...
                'value'     =>  isset($value['min_quantity']) ? $value['min_quantity'] : '',
            ])
...
//                    'label_attr' => ['class' => 'control-label'],
//                    'choices'    => array_combine($langid, $lang),
//                ]);
            }
            $this
