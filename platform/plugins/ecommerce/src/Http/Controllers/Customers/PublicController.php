...
class PublicController extends Controller
{
    ...
    protected $bookedServiceRepository;
     ...
     * @param BookedServiceInterface $bookedServiceRepository
     */
    public function __construct(
        ...
        BookedServiceInterface $bookedServiceRepository,
    ...
    public function getEditAccount()
    {
        ...
        $customer_address = DB::table('ec_customer_addresses')->where('customer_id',auth('customer')->id())->get();
        ...
        return Theme::scope('ecommerce.customers.edit-account', ['countries_list' => $countries_list,'customer_address' => $customer_address], 'plugins/ecommerce::themes.customers.edit-account')
            ->render();
    }
    ...
    public function postChangePassword(UpdatePasswordRequest $request, BaseHttpResponse $response)
    {
        ...
        if(!preg_match("#[\W]+#",$request->input('password'))) {
            $passwordErr = 1;
        }
        if($passwordErr){
            return $response
                ->setError()
                ->setMessage(trans('core/acl::users.password_format_validation'));
        }
    ...
    public function getListServices(Request $request)
    {
        ...
        $services = $this->bookedServiceRepository->advancedGet([
            'condition' => [
                'user_id'     => auth('customer')->id(),
            ],
            'paginate'  => [
        ...
        Theme::breadcrumb()
            ->add(__('Home'), route('public.index'))
            ->add(__('Services'), route('customer.services'));

        return Theme::scope('ecommerce.customers.services.list', compact('services'),
            'plugins/ecommerce::themes.customers.services.list')->render();
    }
    ...
    public function getViewService($id)
    {
        SeoHelper::setTitle(__('Service detail :id', ['id' => get_order_code($id)]));

        $service = $this->bookedServiceRepository->getFirstBy(
            [
                'id'      => $id,
                'user_id' => auth('customer')->id(),
            ],
        ...
        Theme::breadcrumb()->add(__('Home'), route('public.index'))
            ->add(__('Service detail :id', ['id' => get_order_code($id)]),
                route('customer.services.view', $id));

        return Theme::scope('ecommerce.customers.services.view', compact('service'),
            'plugins/ecommerce::themes.customers.services.view')->render();
    }
