...
class OrderController extends BaseController
{
    ...
    /* public function appOrderHistory(){
        $input = request()->all();
        $customer = Customer::where('id', $input['user_id'])->first();
        if($customer) {
            $order = Order::select('id','status','created_at')->where('user_id', $input['user_id'])->where('is_finished', 1)->get();
            foreach($order as $key => $values) {
                $order[$key]['orderdate'] =  date('d-m-Y',strtotime($values['created_at']));
                $order[$key]['estimated_delivery_date'] =  date('d-m-Y',strtotime($values['created_at'].' + 7 days'));;

                $orderProducts = OrderProduct::where('order_id',$values['id'])->get();
                $product = [];
                foreach ($orderProducts as $opKey => $opValue) {
                    $productDetail = Product::where('id',$opValue->product_id)->first();
                    ...
                    if(!empty($productDetail)) {
                        ...
                        $product[$opKey]['image'] = $productDetail->app_image;
                    } else {
                        $product[$opKey]['image'] = [];
                    }

                    $product[$opKey]['id'] = $opValue->product_id;
                    $product[$opKey]['price'] = $opValue->price;
                    $product[$opKey]['quantity'] = $opValue->qty;
                    $product[$opKey]['title'] = $opValue->product_name;
                    $product[$opKey]['status'] = $values->status;

                }
                $order[$key]['product'] = $product;
            }

            return returnResponse(true, 200, '', __('alias.controller.auth.apAuthController.OrderHistoryData'), $order);
        } else {
    ...
    /*    public function appOrderHistory(){
        ...
        $status = false;
         ...
         foreach($order as $key => $values) {
                    $order[$key]['orderdate'] =  date('d-m-Y',strtotime($values['created_at']));
                    $order[$key]['estimated_delivery_date'] =  date('d-m-Y',strtotime($values['created_at'].' + 7 days'));
            $order[$key]['isReturnRequested'] = $status;
                    $orderProducts = OrderProduct::where('order_id',$values['id'])->get();
                    $product = [];
                    foreach ($orderProducts as $opKey => $opValue) {
                        $productDetail = Product::where('id',$opValue->product_id)->first();

                        $prductReturn = ReturnProduct::where('product_id', $opValue->product_id)->first();
                       if(!empty($prductReturn->id)){
                             $status = true;
                        }else{
                           $status = false;
                        }
                        ...
                        $product[$opKey]['title'] = $opValue->product_name;
                        $product[$opKey]['status'] = $values->status;
                        $product[$opKey]['isReturnRequested'] = (@$status)?$status:false;
        ...
        if ($customer) {
            ...
            foreach ($order as $key => $values) {
                ...
                foreach ($orderProducts as $opKey => $opValue) {
                    ...
                    $prductReturn = ReturnProduct::where('product_id', $opValue->product_id)->where('user_id', $input['user_id'])->where('order_id',$opValue->order_id)->first();
                    $status = false;
                    if (!empty($prductReturn->id)) {
                        $status = true;
                    }
                    ...
                    $product[$opKey]['quantity'] = $opValue->qty;
                    $product[$opKey]['title'] = $opValue->product_name;
                    $product[$opKey]['isReturnRequested'] = $status;
                }
                $orderReturn = ReturnProduct::where('order_id', $values->id)->where('user_id', $input['user_id'])->first();
                $status = false;
            ...
            return returnResponse(true, 200, '', __('alias.controller.auth.apAuthController.OrderStatusUpdated'), $order);
        } else {
            return returnResponse(false, 401, '', __('alias.controller.auth.apAuthController.userNotFound'));
        }
    }
    ...
    public function appOrderCreate(){
        ...
        if($customer) {
            $orderData =  array(
                'amount'               => $input['amount'] + $input['shipping_amount'] - $input['discount_amount'],
            ...
            //echo "<pre>";print_r($input['product_id']);die;
            foreach ($input['product_id'] as $key => $product_id) {
                ...
                $productData['qty'] = $input['qty'][$key];
                $productData['order_id'] = $order->id;
                $productData['product_name'] = $productDetail->name;
                $productData['price'] = $input['qty'][$key] * $productDetail->price;
                $productData['weight'] = $productDetail->weight;
                $productData['tax_amount'] = $input['tax_amount'];
                OrderProduct::create($productData);
            }

            return returnResponse(true, 200, '', __('alias.controller.auth.apAuthController.OrderCreate'), $order->id);
        } else {
            return returnResponse(false, 401, '', __('alias.controller.auth.apAuthController.userNotFound'));
        }
    }
    ...
    public function downloadInvoice(){
        ...
        if($customer) {
            ...
            if($order){
                $invoice = OrderHelper::generateInvoice($order);
                $data =  'http://15.184.143.70/storage/invoices/invoice_order_' . get_order_code($order->id) . '.pdf';
                return returnResponse(true, 200, '', __('alias.controller.auth.apAuthController.DownloadInvoice'),$data);
            }
        } else {
            return returnResponse(false, 401, '', __('alias.controller.auth.apAuthController.userNotFound'));
        }
    ...
    public function orderPlacedConfirmation(){
        $input = request()->all();
        $customer = Customer::where('id', $input['user_id'])->first();
        if($customer) {
            $order = Order::where('id',$input['order_id'])->first();
            if($order){
                $order->order_number = "";
                $payment = Payment::where(['user_id'=>$input['user_id'],'order_id'=>$input['order_id']])->first();
                if($payment) {
                    $order->order_number = $payment->charge_id;
                }
                return returnResponse(true, 200, '', __('alias.controller.auth.apAuthController.OrderPlacedConfirmation'),$order);
            }
        } else {
            return returnResponse(false, 401, '', __('alias.controller.auth.apAuthController.userNotFound'));
        }
    ...
    public function orderHistoryDetails() {
        ...
        if($customer) {
            $order = Order::where('id',$input['order_id'])->first();
            if($order){
                $store_data = Store::where('id',$order['store_id'])->first();
                if(!empty($store_data)) {
                    $order['store_name'] = $store_data->name;
                } else {
                    $order['store_name'] = "";
                }
                return returnResponse(true, 200, '', __('alias.controller.auth.apAuthController.OrderHistoryDetails'),$order);
            }
        } else {
            return returnResponse(false, 401, '', __('alias.controller.auth.apAuthController.userNotFound'));
        }
        ...
        if ($customer) {
            ...
            if ($store) {
                ...
                foreach ($orders as $key => $values) {
                    $orders[$key]['orderdate'] =  date('d-m-Y', strtotime($values['created_at']));
                    $orders[$key]['estimated_delivery_date'] =  date('d-m-Y', strtotime($values['created_at'] . ' + 7 days'));
                    $ordersProducts = OrderProduct::where('order_id', $values['id'])->get();
                    $product = [];
                    foreach ($ordersProducts as $opKey => $opValue) {
                        $productDetail = Product::where('id', $opValue->product_id)->first();
                        ...
                        if (!empty($productDetail)) {
                            ...
                            if (!empty($productDetail->images)) {
                                foreach ($productDetail->images as $iKey => $iValues) {
                                    $product_images[$iKey] = url("storage/" . $iValues);
                                }
                            }
                            $productDetail->app_image = $product_images;
                            $product[$opKey]['image'] = $productDetail->app_image;
                    ...
                    $ordersReturn = ReturnProduct::where('order_id', $values->id)->where('user_id', $input['user_id'])->first();
                    $status = false;
                    if (!empty($ordersReturn)) {
                        $status = true;
                    }
                    $orders[$key]['isReturnRequested'] = $status;
                    $orders[$key]['product'] = $product;
                }
                return returnResponse(true, 200, '', __('alias.controller.auth.apAuthController.OrderMyList'), $orders);
            } else {
    ...
    public function appOrderStatusUpdate(Request $request)
    {
        $input = request()->all();
        $customer = Customer::where('id', $input['user_id'])->first();
        if ($customer) {
            ...
            if ($store) {
                ...
                if($order)
                {
                    ...
                    return returnResponse(false, 401, '', __('alias.controller.auth.apAuthController.orderNotFound'));
