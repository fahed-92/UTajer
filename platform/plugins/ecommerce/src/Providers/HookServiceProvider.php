...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        $this->app->booted(function () {
            ...
            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                $items = app(StoreInterface::class)->count([
                    'status'       => BaseStatusEnum::PUBLISHED,
                ]);
                return (new DashboardWidgetInstance)
                    ->setType('stats')
                    ->setPermission('marketplace.store.index')
                    ->setTitle(trans('plugins/marketplace::store.name'))
                    ->setKey('widget_total_4')
                    ->setIcon('far fa-file-alt')
                    ->setColor('#75b6f9')
                    ->setStatsTotal($items)
                    ->setRoute(route('marketplace.store.index'))
                    ->init($widgets, $widgetSettings);
            }, 5, 2);
            ...
            if (defined('PAYMENT_ACTION_PAYMENT_PROCESSED')) {
                add_action(PAYMENT_ACTION_PAYMENT_PROCESSED, function ($data) {
                    ...
                    $bookedServiceIds = (array)$data['booked_service_id'];
                    if ($orderIds) {
...
//                            PaymentHelper::storeLocalPayment($data);
                    ...
                    } else if($bookedServiceIds) {
                        $actualBookedServiceIds = array_values($bookedServiceIds);

                        foreach ($actualBookedServiceIds[0] as $id){
                            $serviceBooking = ServiceBooking::find($id);
                            $data['amount'] = $serviceBooking->amount;
                            $data['service_id'] = $serviceBooking->service_id;
                            $data['currency'] = strtoupper(cms_currency()->getDefaultCurrency()->title);
                            PaymentHelper::storeLocalPayment($data);
                        }
                        PaymentHelper::storeLocalPayment($data);
        ...
        theme_option()
            ->setSection([
                ...
                'fields'     => [
                    ...
                    [
                        'id'         => 'max_service_filter_price',
                        'type'       => 'number',
                        'label'      => trans('plugins/ecommerce::ecommerce.theme_options.max_price_filter'),
                        'attributes' => [
                            'name'    => 'max_service_filter_price',
                            'value'   => 10000,
                            'options' => [
                                'class' => 'form-control',
                            ],
