...
class OrderHelper
{
    ...
    public function generateInvoice($order)
    {
        ...
        $invoice = $folderPath . '/invoice_order_' . get_order_code($order->id) . '.pdf';
    ...
    public function handleAddToCartService($service, $request)
    {
        $parentService = $service->service;

        $image = $parentService->image;

        $img = json_decode($parentService->image,true);
        if ($img == null){
            $img['imgPath'] = '';
        }
        /**
         * Add cart to session
         */
        ServiceBooking::instance('service_booking')->add(
            $service->id,
            $parentService->name,
            $request->input('qty', 1),
            $service->service->price,
            [
                'image'      => RvMedia::getImageUrl($img['imgPath'], 'thumb', false, RvMedia::getDefaultImage()),
                'attributes' => '',
                'taxRate'    => '',
                'extras'     => $request->input('extras', []),
            ]
         ...
         */
        $cartItems = [];
        foreach (ServiceBooking::instance('service_booking')->content() as $item) {
            array_push($cartItems, $item);
        }
