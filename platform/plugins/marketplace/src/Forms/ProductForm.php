...
class ProductForm extends BaseProductForm
{
    ...
    public function buildForm()
    {
        ...
        $this
            ...
            ->add('sample', 'checkbox', [
                'label'         => trans('core/base::forms.is_sample'),
                'label_attr'    => ['class' => 'control-label'],

            ])
            ->add('min_quantity', 'number', [
                'label'      => trans('plugins/ecommerce::products.form.min_quantity'),
                'label_attr' => ['class' => 'text-title-field required'],
                'attr'       => [
                    'placeholder'  => trans('core/base::forms.min_quantity_placeholder'),
                    'data-counter' => 120,
                ],
            ])
            ->add('categories[]', 'categoryMulti', [
                'label'      => trans('plugins/ecommerce::products.form.categories'),
                'label_attr' => ['class' => 'control-label categorySelection'],
//                'choices'    => get_product_categories_with_children(),
                'choices'    => array_merge(get_product_categories_with_children(),['other' => 'other_category']),
                'value'      => old('categories', $selectedCategories),
                'attr'       => ['class' => 'categorySelection'], // Set class for the select element
                'choice_attr' => function ($category, $key, $value) {
                    return ['class' => 'category-option-' . $key];
                },
            ])

            ->add('other_category', 'text', [
                'label'      => trans('plugins/ecommerce::products.form.other_category'),
                'label_attr' => ['class' => 'control-label', 'placeholder'=> 'Other Category'],
                'attr'       => ['id' => 'otherCategoryInput'], // Initially hide the text box
            ])
