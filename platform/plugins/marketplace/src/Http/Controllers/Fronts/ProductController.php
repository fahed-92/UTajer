...
class ProductController extends BaseController
{
    ...
    public function store(
        ...
        if ($request->other_category != null){
            $category = Category::create([
                'parent_id' => 0,
                'name' => $request->other_category,
                'description' => json_encode([
                    'Category' => $request->other_category,
                    'Customer Id' =>  auth('customer')->user()->id,
                ]),
                'author_id'=> auth('customer')->user()->id,
                'order' => 0,
                'is_featured' => 0,
                'status' => 'published',
                'colour' => '#FFFF00',
            ...
            $index = array_search("other_category", $categories);
            if ($index !== false) {
                $newValue = $category->id; // Replace with the desired value
                $categories[$index] = (string) $newValue;
//                 dd($request->all(),$index , $categories );
