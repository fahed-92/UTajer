<div class="ps-cart__content">
    ...
    @elseif (ServiceBooking::instance('service_booking')->count() > 0)
        <div class="ps-cart__items">
            <div class="ps-cart__items__body">
                @php
                            ...
                            <div class="ps-product--cart-mobile">
                                <div class="ps-product__thumbnail">
                                    <a href="{{ url('service-details/'.$service->service_id) }}">
                                        ...
                                        @else
                                            <img src="{{url('storage/general/newsletter-150x150.jpg')}}" alt="">
                                        @endif
                                    </a>
                                </div>
                                <div class="ps-product__content">
                                    <a class="ps-product__remove remove-cart-item" href="#" data-url="{{ route('public.service-booking.remove', $service->id) }}">
                                        <i class="icon-cross"></i>
                                    </a>
                                    <a href="{{ url('service-details/'.$service->service_id) }}"> {{ $service->service->name }}</a>
                                    <p class="mb-0">
                                        <small>
                                            <span class="d-inline-block">{{ $service->date}} {{ $service->start_time}} - {{ $service->end_time}} </span>
                                            <br>
                                            <span class="cart-price">
                                                   @if ($service->service->price_type == 1)
                                                    {{ format_price($service->service->price) }} / Per Day
                                                @elseif ($service->service->price_type == 2)
                                                    {{ format_price($service->service->price) }} / Per Hour
                                                @endif
                                            </span>
                                        </small>
                                    </p>
        ...
        <div class="ps-cart__footer">
            @if (EcommerceHelper::isTaxEnabled())
                <h5>{{ __('Sub Total') }}:<strong>{{ format_price(ServiceBooking::instance('service_booking')->rawSubTotal()) }}</strong></h5>
                <h3>{{ __('Total') }}:<strong>{{ format_price(ServiceBooking::instance('service_booking')->rawSubTotal() + ServiceBooking::instance('service_booking')->rawTax()) }}</strong></h3>
            @else
                <h3>{{ __('Sub Total') }}:<strong>{{ format_price(ServiceBooking::instance('service_booking')->rawSubTotal()) }}</strong></h3>
            @endif
            <figure>
                <a class="ps-btn" href="{{ route('public.cart') }}">{{ __('View Service') }}</a>
                @if (session('tracked_start_checkout'))
                    <a href="{{ route('public.checkout.information', session('tracked_start_checkout')) }}" class="ps-btn">{{ __('Checkout') }}</a>
    ...
    @else
        <div class="ps-cart__items ps-cart_no_items">
            <span class="cart-empty-message">{{ __('No products & Service in the cart.') }}</span>
        </div>
    @endif
</div>
