                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="form-group--number product__qty">
                                                @php
                                                    $sessionOrderId = null;
                                                    $sessionOrder = session()->get('sampleOrderId');
                                                    if ($sessionOrder) {
                                                    $sessionOrderId = (int)$sessionOrder['orderId'];
                                                    }
                                                @endphp
                                                @if($sessionOrderId !=  (($product->is_variation || !$product->defaultVariation->product_id) ? $product->id : $product->defaultVariation->product_id))
                ...
                @elseif(count($services) > 0)
                    <div class="table-responsive">
                        <table class="table ps-table--shopping-cart">
                            ...
                            <tbody>
                            @foreach(ServiceBooking::instance('service_booking')->content() as $key => $serviceItem)
                                @php
                                    $service = $services->find($serviceItem->id);
                           $img = json_decode($service->service->image,true);
                                @endphp

                                @if (!empty($service))
                                    <tr>
                                        <td>
                                            <input type="hidden" name="items[{{ $key }}][rowId]" value="{{ $serviceItem->rowId }}">
                                            <div class="ps-product--cart">
                                                <div class="ps-product__thumbnail">
                                                    <a href="{{ url('service-details/'.$service->service_id) }}">
                                                        @if($img)
                                                            <img src="{{ url('storage/'. $img['imgPath']) }}" alt="{{ $service->name }}" />
                                                        @else
                                                            <img src="{{url('vendor/core/core/base/images/placeholder.png')}}" alt="">
                                                        @endif
                                                    </a>
                                                </div>
                                                <div class="ps-product__content">
                                                    <a href="{{ url('service-details/'.$service->service_id) }}">{{ $service->service->name }}</a>

                                                    <p class="mb-0"><small>{{ $serviceItem->options['attributes'] ?? '' }}</small></p>
                                                    @if (!empty($serviceItem->options['extras']) && is_array($serviceItem->options['extras']))
                                                        @foreach($serviceItem->options['extras'] as $option)
                                                            @if (!empty($option['key']) && !empty($option['value']))
                                                                <p class="mb-0"><small>{{ $option['key'] }}: <strong> {{ $option['value'] }}</strong></small></p>
                                                            @endif
                                                        @endforeach
                                        ...
                                        <td class="price text-center">
                                            <div class="product__price">
                                                @if ($service->service->price_type == 1)
                                                    <span>{{ format_price($service->service->price) }} </span><small> / Per Day</small>
                                                @elseif ($service->service->price_type == 2)
                                                    <span>{{ format_price($service->service->price) }}</span> <small>  / Per Hour</small>
                                                @endif
                                            </div>
                                        </td>
                                        <td class="text-center">{{ $service->date }}</td>
                                        <td><a href="#" data-url="{{ route('public.service-booking.remove', $serviceItem->rowId) }}" class="remove-cart-button"><i class="icon-cross"></i></a></td>
                                    </tr>
                                @endif
                            @endforeach
                            </tbody>
            ...
            <div class="ps-section__footer">
                <div class="row">
                    ...
                    <div class="col-lg-6 col-md-12 col-sm-12 ">
                        <div class="ps-block--shopping-total">
                            <div class="ps-block__header">
                                <p>{{ __('Subtotal') }} <span> {{ format_price(ServiceBooking::instance('service_booking')->rawSubTotal()) }}</span></p>
                            </div>
                            @if (EcommerceHelper::isTaxEnabled())
                                <div class="ps-block__header">
                                    <p>{{ __('Tax') }} <span> {{ format_price(ServiceBooking::instance('service_booking')->rawTax()) }}</span></p>
                                </div>
                            @endif
                            @if ($couponDiscountAmount > 0 && session('applied_coupon_code'))
                                <div class="ps-block__header">
                            ...
                            @endif
                            <div class="ps-block__content">
                                <h3>{{ __('Total') }} <span>{{ ($promotionDiscountAmount + $couponDiscountAmount) > ServiceBooking::instance('service_booking')->rawTotal() ? format_price(0) : format_price(ServiceBooking::instance('service_booking')->rawTotal() - $promotionDiscountAmount - $couponDiscountAmount) }}</span></h3>
                            </div>
                        </div>
                        <a class="ps-btn d-inline-block float-left" href="{{ route('public.services') }}"><i class="icon-arrow-left"></i> {{ __('Back to Services') }}</a>
                        <a class="ps-btn ps-btn d-inline-block float-right" href="{{ route('public.service.checkout', OrderHelper::getOrderSessionToken()) }}">{{ __('Proceed to checkout') }} <i class="icon-arrow-right"></i></a>
                    </div>
                </div>
            </div>
        @endif
