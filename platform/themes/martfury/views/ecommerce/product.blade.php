                ...
                <div class="ps-product--detail ps-product--fullwidth">
                    <div class="ps-product__header">
                        ...
                        <div class="ps-product__info">
                            <div  class="justify-content-between align-items-center py-2 d-flex">
                                <h1 class="mb-0">{{ $product->name }}</h1>
                                <form class="add-to-cart-form" method="POST" action="{{ route('public.cart.add-to-cart') }}">
                                    @csrf
                                    <input class="form-control qty-input" type="hidden" name="qty" value="1" placeholder="1" >
                                    ...
                                    <input type="hidden" name="sample_order" value="sample">
                                    @if (EcommerceHelper::isCartEnabled())
                                        ...
                                        @endphp
                                        @if(($sessionOrderId ==  (($product->is_variation || !$product->defaultVariation->product_id) ? $product->id : $product->defaultVariation->product_id)) || (Cart::instance('cart')->count() > 0))
                                            <div class="text-warning btn-shopping-cart sampleTextProduct">Can't order sample product</div>
                                        @else
                                            @if($product->sample == 1)
                                                <button class="ps-btn btn-shopping-cart sampleTextButton @if ($product->isOutOfStock()) btn-disabled @endif" name="sample_order" type="submit" @if ($product->isOutOfStock()) disabled @endif>{{ __('Sample Order') }}</button>
                                            @endif
                                        @endif
                                    @endif
                                </form>
                            ...
                            <h4 class="ps-product__price @if ($product->front_sale_price !== $product->price) sale @endif"><span>{{ format_price($product->front_sale_price_with_taxes) }}</span> @if ($product->front_sale_price !== $product->price)/ Piece  @endif</h4>
                                ...
                                <div class="ps-product__shopping">
                                    <figure>
                                        ...
                                        <div class="form-group--number product__qty">
                                            ...
                                            <input class="form-control qty-input" type="number" name="qty" value="{{ $product->min_quantity }}" placeholder="1" >
                                        </div>
                                    </figure>
                                    <input type="hidden" name="id" class="hidden-product-id" value="{{ ($product->is_variation || !$product->defaultVariation->product_id) ? $product->id : $product->defaultVariation->product_id }}"/>
...
@endif -->
                    ...
                    <div class="ps-product__content ps-tab-root">
                        <ul class="ps-tab-list">
                            ...
                            <li><a href="#return-policy">{{ __('Return Policy') }}</a></li>
                            @if (EcommerceHelper::isReviewEnabled())
                        ...
                        <div class="ps-tabs">
                            ...
                            <div class="ps-tab" id="return-policy">
                                <div class="ps-document">
...
@endif -->
        ...
        @php
            $crossSellProducts = get_cross_sale_products($product, 0);
        @endphp
